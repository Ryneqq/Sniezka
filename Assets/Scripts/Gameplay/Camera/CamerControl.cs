...
public class CamerControl : MonoBehaviour {
	...
	void Update () 
	{
        if (!relased)
        {
            ...
            offset.x += impact.x;
            offset.y += impact.y;
            transform.position = new Vector3(kulka.transform.position.x + offset.x, kulka.transform.position.y + offset.y, offset.z - kulka.transform.localScale.x);
        }
    }
